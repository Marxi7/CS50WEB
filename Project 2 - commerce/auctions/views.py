...
def bid_won(request):
    # Who the user is
    user = request.user
    bids_won_by_user = Listings.objects.filter(winner=user)
    return render(request, "auctions/bids_won.html", {
        "bids_won_by_user": bids_won_by_user
    })
...
def show_listing_by_category(request):
    if request.method == "POST":
        ...
        if category_selected == "All_Categories":
            ...
            active_listings = Listings.objects.filter(isActive=True, category=category)
            everyCategories = Category.objects.all()
        return render(request, "auctions/index.html", {
            "active_listings": active_listings,
            "everyCategories": everyCategories,
...
def listing_page_info(request, id):
    listing_info = Listings.objects.get(id=id)
    ListingInUserWatchlist = request.user in listing_info.watchlist.all()
    item_comments = Comments.objects.filter(listing=listing_info)
    user = request.user
    ...
    last_bid_on_this_listing = Bids.objects.filter(listing=listing_info).last()
...
def close_auction(request, id):
    ...
    last_bid_on_this_listing = Bids.objects.filter(listing=listing).last()
...
def comment(request, id):
    user = request.user
    listing_selected = Listings.objects.get(id=id)
...
def bid(request, id):
    ...
    bids_on_this_listing = Bids.objects.filter(listing=listing_info)
    if bids_on_this_listing.exists():
        last_bid_on_this_listing = Bids.objects.filter(listing=listing_info).last()
...
def create_listing(request):
    if request.method == "GET":
        ...
        # Creating the new listing by adding the data into the class Listings
        ...
        # Redirecting to the index page where we will see the listings appear
        return HttpResponseRedirect(reverse(index))